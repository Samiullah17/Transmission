    ...
    public function addNTransmission(Request $request)
    {
        ...
        if ($order != null) {
            ...
            if ($order->status == 0) {
                ...
                return response()->json(['message' => 'مخابره په بریالیتوب سره ثبت شوه', 'tra' => $tra, 'order' => $order, 'status' => true]);
            } else {
                return response()->json(['message' => 'غوښتنه/آرډر پروګرام شوی مخابره نشی پکی اضافه کوی!!!', 'status' => false]);
            }
        } else {
            return response()->json(['message' => 'ستونزه ده وروسته بیا کوشش وکړی!!!', 'status' => false]);
        }
    }
